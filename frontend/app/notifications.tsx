...
export default function NotificationsScreen() {
  ...
  const renderNotification = ({ item }: { item: any }) => {
    const getIcon = () => {
      switch (item.type) {
        ...
        case 'friend_accept': return <CheckCircle2 size={20} color="#10b981" />;
        case 'new_message': return <MessageCircle size={20} color="#8b5cf6" />;
        case 'new_room': return <MapPin size={20} color="#f59e0b" />;
    ...
    return (
      <TouchableOpacity
        onPress={handlePress}
        className={`mb-3 flex-row items-center rounded-3xl p-4 border ${item.is_read ? 'bg-card border-border/50' : 'bg-primary/5 border-primary/20'}`}
      >
        <View className={`h-12 w-12 items-center justify-center rounded-full ${item.is_read ? 'bg-secondary' : 'bg-primary/10'}`}>
          {getIcon()}
        </View>
        <View className="ml-4 flex-1">
          <Text className={`text-base font-bold ${item.is_read ? 'text-foreground' : 'text-primary'}`}>
            {item.title}
          </Text>
          <Text className="text-sm text-muted-foreground mt-0.5" numberOfLines={2}>
            {item.content}
          </Text>
          <Text className="text-[10px] text-muted-foreground mt-2 uppercase tracking-tighter">
  ...
  return (
    <SafeAreaView className="flex-1 bg-background" edges={['top']}>
      <Stack.Screen options={{ title: 'Notifications', headerTransparent: true, headerTitle: '' }} />
      <View className="flex-1 px-6">
        <View className="mb-6 mt-4 flex-row items-center justify-between">
          <View>
            <Text className="text-3xl font-bold text-foreground">Activity</Text>
            <Text className="text-sm text-muted-foreground">Stay updated on your interactions.</Text>
          </View>
        </View>
