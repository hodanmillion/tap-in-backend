...
export default function ChatsScreen() {
  ...
  const renderRoom = ({ item }: { item: any }) => (
    <TouchableOpacity
      ...
      className="mb-4 flex-row items-center rounded-3xl border border-border/50 bg-card p-4 shadow-sm">
      <View
        className={`h-14 w-14 items-center justify-center rounded-full ${item.type === 'private' ? 'bg-primary/10' : 'bg-secondary'}`}>
      ...
      <View className="ml-4 flex-1">
        <View className="flex-row items-center justify-between">
          ...
          <Text className="text-[10px] uppercase text-muted-foreground">{item.type}</Text>
        </View>
    ...
    <SafeAreaView className="flex-1 bg-background" edges={['top']}>
      <View className="flex-1 px-6">
        <View className="mb-6 mt-4">
          ...
          <Text className="text-sm text-muted-foreground">Your conversations and groups.</Text>
        </View>
